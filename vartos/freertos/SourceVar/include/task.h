...
/*
 * vaRTOS EXTENSION PATCHES BELOW ------------------------------------------------
 */
#ifdef USE_VARTOS
/*
 * Task Knob types
 */
typedef unsigned int portKNOB_TYPE;
typedef char portUTILITY_SCALAR_TYPE;

/*
 * Scheduler power consumption and temp function parameter type
 * (currently these are unused)
 */
typedef struct xPOWER_PARAMTERS
{
	char runningTaskID;
} portPOWER_ARGS_TYPE;

typedef struct xTEMP_PARAMTERS
{
	char runningTaskID;
} portTEMP_ARGS_TYPE;

#endif // ifdef USE_VARTOS--------------------------------------------------------
...
// varTOS task create (pdm) (BOTH FUNCTIONS ARE USED HERE, i.e. do not wrap in #ifdef's)
#ifdef USE_VARTOS
#define xTaskCreate( pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority, pxCreatedTask, pvKnob, pvKnobMin, pvKnobMax, pvUtilityScalar ) xTaskGenericCreate( ( pvTaskCode ), ( pcName ), ( usStackDepth ), ( pvParameters ), ( uxPriority ), ( pxCreatedTask ), ( NULL ), ( NULL ), pvKnob, pvKnobMin, pvKnobMax, pvUtilityScalar )

#else 

// standard task create (not varOS). do not use for varOS (pdm):
...
// Varos version (pdm)
void vTaskStartScheduler( unsigned long (*userPowerFunc)(portPOWER_ARGS_TYPE params), unsigned long (userTempFunc)(portTEMP_ARGS_TYPE params), char* tempModel, unsigned short lifetime, unsigned short battery) PRIVILEGED_FUNCTION;
...
/*
 * VarOS version of the task creation function (based on the above) (pdm)
 */
signed portBASE_TYPE xTaskGenericCreate( 
    ...
    portUTILITY_SCALAR_TYPE pvUtilityScalar ) PRIVILEGED_FUNCTION;
 ...
 * for Varos (pdm)
 */
//void optimizeTaskKnobs( void );
//void fitKnobModel( tskTCB task ); (in c file)
unsigned portBASE_TYPE xGetTotalTicks( void );
int xGetMiscVal( void );
