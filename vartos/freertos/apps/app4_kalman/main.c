...
/* Task delays */
#define SENSOR1_DELAY ((portTickType) 200 / portTICK_RATE_MS )
#define SENSOR2_DELAY ((portTickType) 200 / portTICK_RATE_MS )
...
/* Example empty task */
static void vHookKalman( void *pvParameters );
...
int main( void )
{
    ...
    // Generic tasks
	xTaskCreate( vHookKalman, "Task1", configMINIMAL_STACK_SIZE, NULL, mainTASK_PRIORITY, &handleKalman, &kalman_knob, 100, 5000, 1);
    #else
...
static void vHookKalman( void *pvParameters )
{
    ...
    // start freq
    unsigned long kalman_delay = 1000*(1000/portTICK_RATE_MS)/kalman_knob;

    // sensor var
    int sensor_raw;
    float z;

    // kalman variables / arrays
    float dt = ((float)kalman_delay)*((float)portTICK_RATE_MS);
   	...
   	for( ;; )
	{
		...
		vTaskDelayUntil( &xLastExecutionTime, kalman_delay );

        // calculate kalman delay (knob acts as freq., measured in mHz)
        kalman_delay = 1000*(1000/portTICK_RATE_MS)/kalman_knob;

        sensor_raw = getSensorValue();
        z = (float)sensor_raw;

        
        // ----------------- perform kalman routines ----------------------
        // calculate dt
        dt = ((float)kalman_delay)*((float)portTICK_RATE_MS)/1000.0;
        // update A matrix
        A[0][1] = dt;
        // update xp (xp = A*x)
        xp[0] = A[0][0]*x[0] + A[0][1]*x[1];
        xp[1] = A[1][0]*x[0] + A[1][1]*x[1];
        // update Pp (Pp = A*P*A' + Q)
        Pp[0][0] = P[0][0] + dt*P[1][0] + dt*P[0][1] + dt*dt*P[1][1] + Q[0][0];
        Pp[0][1] = P[0][1] + dt*P[1][1];
        Pp[1][0] = P[1][0] + dt*P[1][1];
        Pp[1][1] = P[1][1] + Q[1][1];
        // update K (K = Pp*H'*inv(H*Pp*H' + R));
        //             = Pp*H'*inv(temp)
        temp = Pp[0][0] + (float)(R);
        K[0] = Pp[0][0]*(1.0/temp);
        K[1] = Pp[1][0]*(1.0/temp);
        // update x (xhat) by (x = xp + K*(z-H*xp))
        x[0] = xp[0] + K[0]*(z - xp[0]);
        x[1] = xp[1] + K[1]*(z - xp[0]);
        // update P by (P = Pp - K*H*Pp)
        P[0][0] = Pp[0][0] - K[0]*Pp[0][0];
        P[0][1] = Pp[0][1] - K[0]*Pp[0][1];
        ...
        // print estimates and time
        uputs("\n");
        uputi((int)tick_count);
        uputs(",");
        uputi((int)kalman_knob);
        uputs(",");
        uputi((int)(1000*dt));
        uputs(",");
        uputi(sensor_raw);
