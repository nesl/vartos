...
int main( void )
{
    ...
    PowerModel.energy_joules = 12960; // corresponding to 2 AAA batteries, because fuck modularity
...
float findOptimalDC( unsigned int hours, unsigned int joules ){
    ...
    float sum2 = 0.0;
    float temp;
    float avp,ps,pa,gamma;
    unsigned int i;
    pa = getPa(0);
    ps = getPs(0);
    avp = (float)joules/(hours*3600.0)*1e9;
...
unsigned long getPa(char temp){
    ...
    // @ 21dC Ps = 308uW Pa = 1.166mW
    // @ 45dC Ps = 471uW Pa = 1.329mW
    return(1187500);
}
