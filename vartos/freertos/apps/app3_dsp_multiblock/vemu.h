...
#define AVG_P				(SLP_P + 8)
#define VEMU_EXTRA			(AVG_P + 8)
#define SENSOR_READ			(VEMU_EXTRA + 8)


#define READ_CMD       		(0xD00)
#define EXIT_CMD           	(0xF00)

#define TIME_SCALE_CMD		(READ_CMD + 8)
...
uint32_t vemu_read_extra()
{
	*(uint32_t*)(base_pointer + READ_CMD) = 1;
	return *(uint32_t*)(base_pointer + VEMU_EXTRA);
}

int getSensorValue()
{
	*(uint32_t*)(base_pointer + READ_CMD) = 1;
	return (int)*(uint32_t*)(base_pointer + SENSOR_READ);
}
...
void vemu_read_sensors(vemu_sensors * target) 
{
	*(uint32_t*)(base_pointer + READ_CMD) = 1;
	...
	target->avp = *(uint32_t*)(base_pointer + AVG_P);			
}
...
inline void vemu_time_scale(uint32_t scale) 
{
	*(uint32_t*)(base_pointer + TIME_SCALE_CMD) = scale;
}
