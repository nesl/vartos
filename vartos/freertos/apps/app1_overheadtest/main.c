...
int main( void )
{
    ...
    // Generic tasks
	xTaskCreate( vHookTask1, "Task1", configMINIMAL_STACK_SIZE, NULL, mainTASK_PRIORITY, &handleTask1, &task1_knob, 1, 60000, 1);
...
static void vHookTask1( void *pvParameters )
{
	...
	for( ;; )
	{
        ...
        /*
        uputs("\n");
        uputi((int)(10000*getMyOffset()));
        uputs(",");
        uputi((int)(100000*getMySlope()));
        */
        //uputs("\nslope: ");
        //uputi((int)(1000*getMySlope()));
        //uputs("\noffset: ");
        //uputi((int)(getMyOffset()));
        //uputs("\nknob,runtime: ");
        //uputi((int)task1_knob);
        //uputs(",");
...
unsigned long long vGetTimerValue( void ){
    vemu_start_read();
    // have to bit shift here to prevent overflows (and can't divid 64 bit)
    // we will have to reverse the shift later in code
...
static void readPowTemp ( void *pvParameters ){
    ...
    for( ;; )
    {
        ...
        // print out energy used
        //vemu_read_registers(&r);
        vemu_start_read();
        end_t = vemu_active_time();
        uputi((int)(end_t-start_t));
...
static void checkErrors ( void *pvParameters ){
    ...
    unsigned long long end_t,start_t;
