...
#define POWER_MODEL_ERROR_KP (0.01f) // for 4e-5 dc per 40 mW error
...
static void checkErrors ( void *pvParameters ){
    ...
    unsigned long energy_new, lifetime_new;
    int remaining_time;
    ...
    for( ;; )
    {
        ...
        // make sure we're in second week
        vemu_read_registers(&r);
        total_time = r.at + r.st;
        remaining_time = 3600*PowerModel.lifetime_hours - total_time/1e6;
        if( total_time > 864000000000 ){
        ...
        energy_new = PowerModel.energy_joules - (r.ae + r.se)/1e6 - remaining_time*(predicted_power-average_power)/1e9;

        // change E/L values
        PowerModel.energy_joules = energy_new;
        PowerModel.lifetime_hours = remaining_time/3600;
        // re-optimize dc
        PowerModel.optimalDC = findOptimalDC( PowerModel.lifetime_hours,
                        PowerModel.energy_joules );
        optimizeTaskKnobs( &PowerModel.optimalDC );
        ...
        uputs("\n");
        uputi((int)(average_power - predicted_power));
        uputs("\n");
        uputi((int)energy_new);
        uputs("\n");
        uputi((int)remaining_time);
        ...
        //if( ABS_VAL( (predicted_power - average_power)/predicted_power ) > POWER_MODEL_ERROR_THRESHOLD ){
            ...
            //PowerModel.optimalDC += POWER_MODEL_ERROR_KP*( (predicted_power - average_power)/1000000.0f );
        ...
        // reset temperature histogram
        temp_hist_numpoints = 0;
        for( i=0; i<TEMP_MODEL_NUM_BINS; i++ ){
            temp_hist_windowed[i] = 0;
        }
    }
...
static void checkErrors ( void *pvParameters ){
    ...
    unsigned long ps,pa;
    float average_power, predicted_power;
    float energy_past, energy_present;
    vemu_sensors s;
    float temperature, freq;
    unsigned long energy_new, lifetime_new;
    float total_time;
    float total_time_past = 0.0;
    ...
    for( ;; )
    {
	    ...
	    vemu_read_sensors(&s);
        vemu_read_registers(&r);
        energy_present = (float)(r.ae + r.se);
        total_time = (float)(r.at + r.st) - total_time_past;
        average_power = (float)( 1e3f*(energy_present-energy_past)/(total_time/1e6f) ); // nJ/s = nW
        ...
        // recycle energy counter
        energy_past = (float)(r.ae + r.se);
        total_time_past = (float)(r.at + r.st);
